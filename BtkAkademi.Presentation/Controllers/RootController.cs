        ...
        public async Task<IActionResult> GetRoot([FromHeader(Name = "Accept")] string mediaType)
        {
            if (mediaType.Contains("application/vnd.btkakademi.apiroot"))
            {
                var list = new List<Link>()
                {
                    ...
                    new Link()
                    {
                        Href = _linkGenerator.GetUriByName(HttpContext, nameof(BookController.GetAllBooks), new{}),
                        Rel = "books",
                        Method = "GET"
                    },
                    new Link()
                    {
                        Href = _linkGenerator.GetUriByName(HttpContext, nameof(BookController.CreateOneBook), new{}),
                        Rel = "books",
                        Method = "POST"
                    }
                };
